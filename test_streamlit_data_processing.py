...
st.subheader('analysis of "avl_delay"')
st.dataframe(df_selected.describe())
...
null_values_percentage=df_selected['avl_delay'].isna().sum()/len(df_selected["avl_delay"])*100

st.text(f'avl_delay 有 376924 个空值, 总体大约有{null_values_percentage:.2f}%的空值，看下各站点的空值占比和空间分布')
...
st.text(f'还剩{len(df_selected["to_stop_san"].unique())}个stops')
...
st.markdown('### The avl_delay values:')
...
df_selected = pd.merge(df_selected, average_delay_by_stop, on='to_stop_san', how='left')
...
early_arrival_percentage_1_to_2_min = (df_selected[(df_selected['avl_delay'] < -60) & (df_selected['avl_delay'] > -120)]['avl_delay'].count() / df_selected['avl_delay'].count() * 100)
st.text(f"avl_delay 早到小于2分钟(-120 ~ -60)占比: {early_arrival_percentage_1_to_2_min:.2f}%")

early_arrival_percentage_2_to_3_min = (df_selected[(df_selected['avl_delay'] < -120) & (df_selected['avl_delay'] > -180)]['avl_delay'].count() / df_selected['avl_delay'].count() * 100)
st.text(f"avl_delay 早到小于3分钟(-180 ~ -120)占比: {early_arrival_percentage_2_to_3_min:.2f}%")

early_arrival_percentage_3_to_4_min = (df_selected[(df_selected['avl_delay'] < -180) & (df_selected['avl_delay'] > -240)]['avl_delay'].count() / df_selected['avl_delay'].count() * 100)
st.text(f"avl_delay 早到小于4分钟(-240 ~ -180)占比: {early_arrival_percentage_3_to_4_min:.2f}%")

early_arrival_percentage_4_to_5_min = (df_selected[(df_selected['avl_delay'] < -240) & (df_selected['avl_delay'] > -300)]['avl_delay'].count() / df_selected['avl_delay'].count() * 100)
st.text(f"avl_delay 早到小于5分钟(-300 ~ -240)占比: {early_arrival_percentage_4_to_5_min:.2f}%")
...
st.text(f"  ")
st.text(f"bus delay的定义：A departure is considered unpunctual if it left more than 3 minutes late or more than 1 minute early.")
st.text(f"来源：https://mrapport.sl.se/media/1016/punktlighetsrapp_styrelsen_jan_2009_ny.pdf Page 15 Diagrammet nedan visar andelen avgångar som avgått i tid. En avgång räknas som opunktlig om den avgått mer än 3 minuter sent eller mer än 1 minut tidigt.")

st.text(f"  ")
st.text(f"文献把早到设成0，但早到占比3%")
st.text(f"https://www.tandfonline.com/doi/full/10.1080/13658816.2019.1608997\n")
...
st.text(f"\n 早到都设成0：")
df_selected['adjusted_delay'] = df_selected.apply(lambda row: 0 if row['avl_delay'] < 0 else row['avl_delay'], axis=1)
# fig_hist = px.histogram(df_selected, x='adjusted_delay', nbins=100, title=f'Distribution of adjusted Delay')
# st.plotly_chart(fig_hist)

st.markdown('##### 晚到')
st.text('删掉大于3倍绝对值均值的delay')
df_selected['adjusted_delay'] = df_selected.apply(lambda row: 0 if row['avl_delay'] < 0 else (np.nan if row['avl_delay'] > 3 * abs(row['average_delay']) else row['avl_delay']), axis=1)
# 删除大于3倍均值的行
df_selected = df_selected.dropna(subset=['adjusted_delay'])

st.markdown('  ')
st.markdown('##### jieguo')
fig_hist = px.histogram(df_selected, x='adjusted_delay', nbins=40, title=f'Distribution of adjusted Delay')
st.plotly_chart(fig_hist)

new_average_delay_by_stop =  df_selected.groupby(['to_stop_san'])['adjusted_delay'].mean().reset_index().rename(columns={'adjusted_delay':'adjusted_average_delay'})
# 创建交互式直方图
fig = px.histogram(new_average_delay_by_stop, x='adjusted_average_delay', nbins=40, 
                   title='by stop',
                   labels={'adjusted_average_delay', 'count'},)
